...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.privileged ) {
      EOS_ASSERT( name_str.find( "mvl." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'mvl.'" );
   }
...
void apply_eosio_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "mvl." ) != 0, action_validate_exception,
               "Permission names that start with 'mvl.' are reserved" );
