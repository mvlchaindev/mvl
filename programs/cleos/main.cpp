...
string default_wallet_url = "unix://" + (determine_home_directory() / "mvl-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "mvl.msig", "propose", variant_to_bin( N(mvl.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "mvl.msig", action, variant_to_bin( N(mvl.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "mvl.msig", "invalidate", variant_to_bin( N(mvl.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "mvl.msig", "cancel", variant_to_bin( N(mvl.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "mvl.msig", "exec", variant_to_bin( N(mvl.msig), N(exec), args ) }});
      }
   );
